...
app.post('/api/analyze-gdc-requirements', async (req, res) => {
  try {
    ...
    const analysisResults = await this.performComprehensiveAnalysis(anthropic, files, requirements);
...
app.post('/api/generate-questionnaire', async (req, res) => {
  try {
    ...
    const questionnaire = await this.generateEnhancedQuestionnaire(anthropic, requirements, documentContext);
...
app.post('/api/generate-gold-standard', async (req, res) => {
  try {
    ...
    const goldStandard = await this.generateGoldStandardFramework(anthropic, requirements, analysisResults);
...
async function performComprehensiveAnalysis(anthropic, files, requirements) {
  ...
  try {
    ...
    return this.generateFallbackAnalysis(requirements);
  }
}
...
async function generateEnhancedQuestionnaire(anthropic, requirements, documentContext) {
  ...
  return this.generateSimulatedQuestionnaire(requirements, documentContext);
}
